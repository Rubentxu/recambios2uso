...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/desguace'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: Desguace) {
        return item.id;
    }
    registerChangeInDesguaces() {
        this.eventSubscriber = this.eventManager.subscribe('desguaceListModification', (response) => this.loadAll());
    }
